...
function getUsersFromDatabase(array) {
 const arrayUsers= Array.from(array);// Використовуємо метод `Array.from` для створення масиву користувачів зі списку, елементи якого це об'єкти які міститять
  ...
  return arrayUsers;// Повертаємо масив користувачів
}
...
function removeLastElement(arr) {
  if(arr.length===0){return undefined}// Перевіряємо, чи масив не є порожнім, якщо порожній повертаємо  undefined
  else{arr.pop()}// Використовуємо метод `pop` для видалення останнього елементу з масиву
...
function filterByCondition(arr, condition) {
 const result=arr.filter(condition);// Використовуємо метод `filter` для фільтрації масиву
  return result; // Повертаємо відфільтрований масив
}
...
function checkArray(obj) {
  ...
  {const result=Array.isArray(obj);// Використовуємо метод `Array.isArray` для перевірки, чи є об'єкт масивом
  return result} // Повертаємо результат перевірки
  else{console.log("Помилка: Вхідний об'єкт є null або undefined.");// Якщо об'єкт є null або undefined, виводимо повідомлення про помилку Помилка: Вхідний об'єкт є null або undefined.
...
function createArray(...elements) {
 if(!elements===undefined) // Перевіряємо, чи кількість переданих елементів більше нуля
  {const array=Array.of(...elements);// Використовуємо метод `Array.of` для створення нового масиву з переданих елементів
  return array;}// Повертаємо створений масив
  else{return console.log("ERROR!!!");}// Якщо не передано жодного елементу, виводимо повідомлення про помилку
...
function getElementAtIndex(arr, index) {
  if(arr.lenght===0)// Перевіряємо, чи масив не є порожнім
  {return undefined}// Повертаємо undefined
  else if(index<arr.length){// Перевіряємо, чи індекс знаходиться в межах довжини масиву
  return arr.at(index);}// Повертаємо отриманий елемент
  else{return undefined}// Якщо індекс виходить за межі масиву,повертаємо undefined
}
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  return arrSum} //  Якщо ні повертаємо пустий масив
  else{arrSum = arrSum.concat(arr1,arr2);// Об'єднуємо два масиви за допомогою методу `concat`
  return arrSum.reverse();// Обертаємо отриманий об'єднаний масив за допомогою методу `reverse`
...
function findElementIndexes(arr, element) {
  ...
  else{arr.indexOf(element);// Знаходимо індекс першого знайденого елементу за допомогою методу `indexOf`
  arr.lastIndexOf(element);// Знаходимо індекс останнього знайденого елементу за допомогою методу `lastIndexOf`
  let arrIndex=[];// Створюємо пустий масив індексів знайдених елементів
  if(!(arr.indexOf(element)===-1)){arrIndex.push(arr.indexOf(element));}// Додаємо перший знайдений індекс, якщо елемент знайдено
  if(!(arr.indexOf(element)=== arr.lastIndexOf(element))){arrIndex.push(arr.lastIndexOf(element));}// Додаємо останній знайдений індекс, якщо він відрізняється від першого
  return arrIndex;}// Повертаємо масив індексів
}
...
function copyAndSwapElements(arr, target, start, end) {
  if(target<arr.length&&start<arr.length&&end<arr.length){// Перевіряємо, чи індекси належать межам масиву якщо ні виводимо рядок Неприпустимі індекси
...
if( Array.isArray(arr)&&typeof(condition)=="function"){// Перевірка вхідних параметрів
  return arr.every(condition)}// якщо arr не масив повертаємо false
else{return false}// якщо condition не function повертаємо false
...
function customFill(arr, value, start = 0, end = arr.length) {
  if(start<0||start>arr.length-1||end<0||end>arr.length-1){return false}
  else if(Array.isArray(arr))// Перевірка вхідних параметрів
  {return arr.fill(value,start,end);}// якщо arr не масив повертаємо false
  else{return false}// якщо start та end не входять в діапазон від 0 до довжини масиву arr повертаємо false
...
function customShift(arr) {
  if(!Array.isArray(arr)){return undefined}// перевірка на масив,якщо ні повертаємо undefined
  else if(arr.length===0){return undefined}// перевірка чи масив не пустий,якщо так повертаємо undefined
  else{ const shiftedElement=arr.shift();// видаляємо перший елемент та записуємо його в змінну shiftedElement
...
function customUnshift(arr, ...elements) {
 ...
 arr.unshift(elements[i]);} // при кожній ітерації лічильник зменшуємо на 1 та продовжуємо поки лічильник більше, або дорівнює нулю
...
function customSome(arr, condition) {
  ...
  else if(!typeof(condition)==="function"){return false}// Перевіряємо, чи condition є функцією  якщо ні повертаємо false
  else{return arr.some(condition)}// Використовуємо метод `some` для перевірки умови хоча б для одного елементу масиву
...
function customAt(arr, index) {
  if(!Array.isArray(arr)){return false}// Перевіряємо, чи вхідний параметр є масивом якщо ні повертаємо undefined
  else if(index<0||index>arr.length-1){return undefined}// Перевіряємо, чи індекс виходить за межі масиву якщо ні повертаємо undefined
  else{// Отримуємо елемент масиву за заданим індексом
  const result=arr.at(index);
  switch (typeof(result)){// перевіремо якого типу елемент ми отримали,якщо рядок виведом в консоль -Елемент є рядком,числом - Елемент є числом, об`єктом -Елемент є обєктом
    case "number": console.log('Елемент є числом');break;
    case "string": console.log('Елемент є  рядком');break;
    case "object": console.log('Елемент є об`єктом');break;
  }
return result}
...
function customIncludes(arr, element) {
  if(Array.isArray(arr)){ // Перевіряємо, чи вхідний параметр є масивом
  arr.includes(element);// Використовуємо метод includes для перевірки наявності елемента в масиві
  let filtred=arr.filter((ar)=>ar===element);// За допомогою методу filter перевіряємо скільки разів в масиві зустрічається елемент та виводимо число в консоль
  console.log(filtred.length);
  return  arr.includes(element)}//Повертаємо результат
}
